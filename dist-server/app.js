...
var getAdminDashboardData = async (req, res) => {
  try {
    ...
    const clarityUsers = await db_default.task.groupBy({
      by: ["userId"],
      where: { createdAt: { gte: thirtyDaysAgo } }
    });
    const focusUsers = await db_default.taskBlock.groupBy({
      by: ["taskId"],
      // We need to join task to get userId, but groupBy limited in Prisma.
    ...
    const users = await db_default.user.findMany({
      ...
      orderBy: { createdAt: "desc" },
      take: 50,
      // Back to 50 for management page
      select: {
        id: true,
        name: true,
        email: true,
    ...
    res.json({
      ...
      engagement: {
        clarity: recentTasks,
        // "Clarity" -> Tasks Created
        focus: recentSplits,
        // "Focus" -> AI Splits
        freedom: recentExpenses
        // "Freedom" -> Expenses Tracked
      },
      users
    });
  } catch (error) {

...
var import_resend, import_dotenv, sendEmail;
...
var import_node_cron, startReminderJob;
...
var generateToken = (payload) => {
  ...
  return import_jsonwebtoken.default.sign(payload, JWT_SECRET, options);
};
...
var budget_routes_default = router3;
...
var insights_routes_default = router4;

// server/src/server.ts
import_dotenv2.default.config();
var app = (0, import_express5.default)();
var PORT = process.env.PORT || 5e3;
var allowedOrigins = (process.env.CLIENT_URL || "http://localhost:5173,http://localhost:5174").split(",");
app.use(
...
var clientBuildPath = import_path.default.join(process.cwd(), "dist");
server_default.use(import_express6.default.static(clientBuildPath));
server_default.get("*", (req, res) => {
  if (req.path.startsWith("/api")) {
    res.status(404).json({ error: "API Route not found" });
    return;
  }
  res.sendFile(import_path.default.join(clientBuildPath, "index.html"));
});

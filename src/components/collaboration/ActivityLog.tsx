    ...
    <div className="flex flex-col h-full">
          ...
          comments.map((comment) => {
            ...
            return (
              <div key={comment.id} className={`flex gap-2.5 group ${isMe ? 'flex-row-reverse' : ''}`}>
                <div className="w-7 h-7 rounded-full bg-gradient-to-br from-indigo-400 to-purple-500 flex items-center justify-center text-white text-xs font-bold shrink-0 mt-0.5">
                  {initials}
                </div>
      ...
      <form onSubmit={handlePost} className="flex gap-2 mt-4 pt-3 border-t border-slate-100 dark:border-white/5">
        <input
          ...
          className="flex-1 px-3 py-2 rounded-xl text-sm bg-slate-50 dark:bg-white/5 border border-slate-200 dark:border-white/10 text-slate-900 dark:text-white placeholder-slate-400 focus:outline-none focus:ring-2 focus:ring-indigo-500"
        />
        <button
          type="submit"
          disabled={!newComment.trim() || isPosting}
          className="w-9 h-9 shrink-0 bg-indigo-600 hover:bg-indigo-700 disabled:bg-slate-300 dark:disabled:bg-white/10 text-white rounded-xl flex items-center justify-center transition-colors"
        >

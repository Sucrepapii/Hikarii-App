...
export const LeadCaptureForm: React.FC<LeadCaptureFormProps> = ({
    ...
    const containerClasses = {
        hero: "max-w-xl mx-auto p-8 rounded-3xl bg-white/50 dark:bg-white/5 backdrop-blur-xl border border-indigo-100 dark:border-white/10 shadow-2xl",
            ...
            <form onSubmit={handleSubmit} className={`flex ${variant === 'footer' ? 'flex-col' : 'flex-col sm:flex-row'} gap-3`}>
                <div className="relative flex-1 group">
                    <input
                        ...
                        required
                        className={`w-full px-0 py-3 bg-transparent border-b border-slate-700 dark:border-white/10 text-white focus:border-indigo-500 outline-none transition-all placeholder:text-slate-600 font-medium`}
                    />
                    <div className="absolute bottom-0 left-0 w-0 h-0.5 bg-gradient-to-r from-indigo-500 to-purple-500 group-focus-within:w-full transition-all duration-500"></div>
                </div>
                ...
                <Button
                    ...
                    className={`
                        ${variant === 'footer'
                            ...
                            : 'rounded-xl px-6 h-12 bg-indigo-600 hover:bg-indigo-500 text-white'

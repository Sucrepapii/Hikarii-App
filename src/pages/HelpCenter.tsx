            ...
            <main className="flex-grow pt-32 px-6 max-w-6xl mx-auto w-full pb-20">
                    ...
                    <div className="max-w-2xl mx-auto relative group">
                        ...
                        <div className="relative">
                            <input
                                ...
                                className="w-full px-8 py-6 pl-16 rounded-[2rem] bg-[#0D0F1A] border border-white/10 focus:border-primary-500/50 focus:outline-none transition-all text-lg shadow-2xl placeholder:text-slate-600 text-white"
                            />
                            ...
                            <div className="grid gap-4">
                                ...
                                <Link key={article.id} to={`/help/article/${article.slug}`} className="p-6 rounded-2xl bg-white/5 border border-white/10 hover:border-primary-500/30 hover:bg-white/10 transition-all cursor-pointer flex items-center justify-between group shadow-sm hover:shadow-md">
                                    <div>
                                        ...
                                        <h3 className="text-xl font-bold mb-1 group-hover:text-primary-400 transition-colors">{article.title}</h3>
                    ...
                    <div className="mb-20">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                            {popularArticles.map((article) => (
                                <Link key={article.id} to={`/help/article/${article.slug}`} className="p-4 rounded-xl bg-white/5 border border-white/5 hover:border-primary-500/30 hover:bg-white/10 transition-colors cursor-pointer flex items-center justify-between group">
                                    <span className="text-sm font-medium text-slate-300 group-hover:text-primary-400">{article.title}</span>
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-24">
                            <Link to="/help/category/Getting%20Started" className="p-8 rounded-[2rem] bg-[#0D0F1A] border border-white/[0.06] hover:border-primary-500/30 transition-all duration-300 group block">
                                <div className="w-12 h-12 rounded-2xl bg-primary-500/10 flex items-center justify-center mb-6 border border-primary-500/20 shadow-lg shadow-primary-500/10 group-hover:scale-110 transition-transform">
                                ...
                                {getCount('Getting Started') > 0 && (
                                    <span className="text-primary-400 text-xs font-black tracking-widest uppercase">{getCount('Getting Started')} Articles</span>
                                )}
                            </Link>
                ...
                <div className="bg-[#0D0F1A] border border-white/[0.06] rounded-[2.5rem] p-12 text-center relative overflow-hidden shadow-2xl">
                    ...
                    <div className="relative z-10">
                        ...
                        <Button onClick={() => navigate('/contact')} size="lg" className="bg-gradient-to-r from-primary-600 to-accent-600 hover:from-primary-500 hover:to-accent-500 text-white border-0 px-10 h-14 rounded-xl font-black tracking-widest uppercase text-xs shadow-xl shadow-primary-500/25 transition-all hover:scale-105">Contact Support</Button>
                    </div>
                </div>
            </main>
            <Footer />
